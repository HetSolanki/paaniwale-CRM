...
export const columns1: ColumnDef<Customer>[] = [
        ...
        cell: ({ row }) => {
            ...
            return (
                <div className="flex flex-col">
                    <span className="capitalize font-medium">{cid?.cname}</span>
                    {/* Show payment date on mobile as subtitle */}
    ...
    },
    {
        ...
        cell: ({ row }) => (
            <div className="font-semibold whitespace-nowrap">₹{row.getValue("amount")}</div>
        ),
    },
    {
        accessorKey: "payment_date",
        header: () => <div className="text-left font-semibold hidden md:table-cell">Date</div>,
        cell: ({ row }) => (
            <div className="capitalize whitespace-nowrap hidden md:table-cell">{row.getValue("payment_date")}</div>
        ),
        meta: {
    ...
    },
    {
        ...
        header: () => <div className="text-left font-semibold hidden md:table-cell">Mode</div>,
        cell: ({ row }) => {
            const mode = row.original.payment_method || "cash";
            return (
                <div className="items-center gap-1 whitespace-nowrap hidden md:flex">
            ...
            return (
                <div className="flex items-center gap-1 sm:gap-2">
                    {row.getValue("payment_status") === "Received" || row.getValue("payment_status") === "completed" ? (
                        <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800 text-xs">
                            <CheckCircle2 className="h-3 w-3 mr-1" />
                            <span className="hidden sm:inline">{row.getValue("payment_status")}</span>
                            <span className="sm:hidden">Rcvd</span>
                        </Badge>
                    ) : (
                        <Badge className="bg-orange-100 text-orange-700 hover:bg-orange-200 dark:bg-orange-900/30 dark:text-orange-400 border-orange-200 dark:border-orange-800 text-xs">
